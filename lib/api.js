...
function request(apiRequest) {
    ...
    if (api.client == undefined) return Promise.reject(newError(undefined, 'api not initialized'));
        ...
        return api.client.call(apiRequest)
            .then(apiResponse => { return response(apiRequest, apiResponse); })
            .catch(error => {
...
function response(apiRequest, apiResponse) {
	if (apiRequest.raw_response) return apiResponse.data;
	else
		switch (apiRequest.response_type) {
			case 'model':
