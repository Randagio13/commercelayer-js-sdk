...
function response(apiResponse, response_type) {
	if ((apiResponse == undefined) || (response_type == undefined)) return apiResponse;
	else
...
function error(apiError, error_type) {
	if (error_type == undefined) return apiError.response;
	else
		switch (error_type) {
			case 'model':
				if (!apiError.response)
					return Promise.reject(errorResponse(apiError.code, apiError.message));
				else
...
async function all(listFunction, filter = {}, options = {}) {
	...
	// Check response_type (passed as parameter or default, used for final response conversion)
	options.response_type = options.response_type || sdk.response_type;
	if (options.response_type == 'raw') return Promise.reject('Use of raw response_type not allowed in list-all functions: ');
	...
	const response_type = options.response_type;
	...
	if (max_page == 1) return response(firstPage, response_type);
	else pages[0] = firstPage;
	...
	let pagesOut = await Promise.all(pages);

	if (!Array.isArray(pagesOut) && !pagesOut.data) return pagesOut;
    else {
		let out = [];
		pagesOut.forEach(p => { out = out.concat(data(p)) });
