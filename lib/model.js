...
class Resource {
    ...
    setResource(field, res) { if ((res != undefined) && (helper.isApiResource(res) || helper.isApiResourceArray(res))) this['__'.concat(field)] = res; }
	...
	setField(name, value) { if (name) this[name] = value; return this; }
}
...
class CustomerGroup extends Resource {
    static get TYPE() { return 'customer_groups' }
    constructor(id, fields = {}) {
        super(id, CustomerGroup.TYPE);
...
class Import extends Resource {
    static get TYPE() { return 'imports' }
    constructor(id, fields = {}) {
        super(id, Import.TYPE);
...
helper.allElements = () => {
	...
	Object.keys(model).some((cl) => {
		if (!cl.startsWith('helper')) elements.push(cl)
	});
	return elements;
}

helper.newResource = (type, id, fields) => {
...
helper.isApiResource = (res) => {
	return (res && res.isApiResource && (res.isApiResource() === true));
}

helper.isApiResourceArray = (resArray) => {
