...
class QueryExpression extends query.QueryFilter {
    ...
    predicate(type, value, ...fields) {
        if ((fields != undefined) && (value != undefined)) {
            ...
            const key = 'filter[q][' + fields_.join('_or_') + '_' + type + ']';
    ...
    explain(type) { return QueryExpression.explain(type) }

    build(join) {
        let qsMap = super.build(join);
        if (!join) Object.keys(this.predicates).forEach(p => { qsMap[p] = this.predicates[p]; })
        else {
            if (qsMap.length > 0) qsMap += '&';
            qsMap += Object.keys(this.predicates).map(key => { return key + '=' + this.predicates[key] }).join('&');
        }
        return qsMap;
    }
